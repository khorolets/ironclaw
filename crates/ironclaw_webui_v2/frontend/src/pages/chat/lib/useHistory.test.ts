...
test("mergeFullRefresh appends an optimistic bubble with no preceding anchor", () => {
  // The anchor-walk's append fallback is intentional: a preserved message with
  // no anchored predecessor still lands at the end.
  const context = { globalThis: {}, React: createReactStub() };
  vm.runInNewContext(useHistorySourceForTest(), context);
  const { mergeFullRefresh } = context.globalThis.__testExports;
    ...
    [
      {
        id: "pending-first",
        role: "user",
        content: "first message, not yet persisted",
        isOptimistic: true,
        timestamp: "2026-07-11T13:49:00Z",
      },
    ],
  ...
  assert.equal(merged.map((m) => m.id).join(","), "msg-b,pending-first");
});
