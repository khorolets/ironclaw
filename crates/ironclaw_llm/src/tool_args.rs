...
pub(crate) fn parse_tool_call_args(raw: &str) -> Result<Value, ArgsParseError> {
    ...
    // "trailing characters", and on the streaming tool-call path
    // (`NearAiStreamingToolCallState::into_tool_call`) that error fails the whole
    // turn after the model already emitted its reasoning. Take the first complete
    // JSON value and ignore any trailing content; only empty or genuinely
...
mod tests {
    ...
    fn parse_args_ignores_trailing_content_after_first_value() {
        ...
        // as "trailing characters" (which failed the whole streaming turn).
        let result =
            parse_tool_call_args(r#"{"a": 2} x"#).expect("leading object should parse");
